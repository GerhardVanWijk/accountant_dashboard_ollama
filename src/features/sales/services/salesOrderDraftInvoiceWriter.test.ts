...
describe('RpcSalesOrderDraftInvoiceWriter', () => {
  ...
  it('calls the RPC with only { salesOrderLineId, quantity } and re-reads the created invoice', async () => {
    ...
    expect(rpc).toHaveBeenCalledWith('create_invoice_from_sales_order', {
      ...
      p_created_by: 'user-9',
      p_issue_date: null,
      // NORMALIZED_DOCUMENT_LINES_ENABLED is true (activated 2026-09-05), so
      // the RPC's own atomic invoice_lines projection (migration 0062) is on,
      // mirroring the TS projector.
      p_project_lines: true,
    });
