...
import { auditLogAccessService } from '../services';
import { AccessDenied } from './AccessDenied';

export interface PermissionRouteProps {
  feature: string;
  action?: string;
  /** Human label for the Access Log denial row. Defaults to the feature key. */
  area?: string;
  children: ReactNode;
}
 ...
 * catalog (see `src/features/auth/permissionRouteMap.ts`).
 *
 ...
 *
 * A denied render writes ONE `denied_permission` row to the Access Log
 * (`audit_logs_access` via `log_access_event`, deduped server-side).
 */
export function PermissionRoute({ feature, action, area, children }: PermissionRouteProps) {
  const canAccess = useCanAccess(feature, action);

  useEffect(() => {
    if (canAccess) return;
    auditLogAccessService.logDenied(area ?? feature, { feature, action: action ?? 'read' });
  }, [canAccess, area, feature, action]);
