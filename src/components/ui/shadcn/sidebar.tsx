...
/**
 * Root cause of the "expanded sidebar overlays main content" defect (see
 * V0_VISUAL_FIDELITY.md): the original v0 file uses Tailwind v4's
 * round-paren CSS-variable shorthand (`w-(--sidebar-width)`) in several
 * places below. This project runs Tailwind v3, which has no such syntax —
 * v3's JIT scanner doesn't recognize the pattern and silently generates NO
 * CSS for that class at all (unlike card.tsx's M2-era break, this doesn't
 * fail the build; it just produces an empty rule). That left the "sidebar
 * gap" spacer div with no width, so it never pushed <SidebarInset> over,
 * while the actual fixed sidebar box (also missing its width class) sat at
 * left:0 directly on top of the main content. Rewritten to the equivalent
 * v3-native square-bracket form (`w-[var(--sidebar-width)]`) everywhere
 * this pattern appears in this file — same resulting CSS, just spelled in
 * syntax v3 can parse.
 */
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "sidebar-scroll flex min-h-0 flex-1 flex-col gap-0 overflow-auto group-data-[collapsible=icon]:overflow-hidden",
        className
      )}
      {...props}
    />
...
export {
  ...
  SidebarSeparator,
  SidebarTrigger,
  // eslint-disable-next-line react-refresh/only-export-components -- shadcn's own convention: co-exports the useSidebar hook with the components
  useSidebar,
}
