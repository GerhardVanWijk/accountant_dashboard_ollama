-- 0108_seed_new_company_tax_clearing_accounts
-- Tax & Compliance integrity audit — migration-review addendum, follow-up
-- to 0107 (same relationship 0094 has to 0085). AUTHORED, NOT APPLIED.
-- Apply AFTER 0107 (this migration keeps `seed_new_company_accounting` in
-- sync with the two account codes 0107 introduces).
--
-- 0107 backfills 2270 Provisional Tax Payment Clearing / 2520 Dividends
-- Payment Clearing onto every EXISTING company, but
-- `seed_new_company_accounting()` — the function
-- `create_company_and_become_admin()` calls for every BRAND NEW company
-- (migration 0066, last updated for this exact reason by 0094) — was never
-- updated to include them. Left alone, a company created after 0107
-- applies would get a Chart of Accounts missing both clearing accounts,
-- and its very first provisional tax payment or dividend payment/
-- remittance would fail outright with "account not found"
-- (`accountMappingService`/`pay_provisional_tax`/`_post_dividend_transition`
-- all resolve these by code) — the exact same gap 0094 closed for 0085's
-- two codes, caught here by the identical
-- `companyBootstrapMigration.test.ts` static assertion (every
-- `AccountMappingKey` must have a matching code literal inside 0066's seed
-- function's SQL text).
--
-- This migration is a byte-for-byte `create or replace` of 0094's (itself
-- a byte-for-byte `create or replace` of 0066's) `seed_new_company_accounting`,
-- with exactly two rows added to the VALUES list (2270 immediately after
-- 2250 Net Pay Payable and before 2300 Income Tax Payable; 2520
-- immediately after 2510 Dividends Tax Payable and before 2600 Customer
-- Deposits — matching the account-family grouping the rest of the list
-- already follows) — no other line changed. `create or replace function`
-- keeps the existing REVOKE/no-grant posture (0066 revokes execute from
-- public/anon/authenticated entirely — this function is only ever called
-- internally by `create_company_and_become_admin`, itself `security
-- definer`), so no grant statements are needed here.
...
-- Observability, mirroring 0094's own closing check.
do $$
begin
  if to_regprocedure('public.seed_new_company_accounting(uuid, smallint, smallint)') is null then
    raise exception '0108: seed_new_company_accounting was not created';
  end if;
end $$;
