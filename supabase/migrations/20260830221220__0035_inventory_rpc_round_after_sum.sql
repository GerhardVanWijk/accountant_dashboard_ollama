-- 0035_inventory_rpc_round_after_sum
...
--
--
-- `create or replace` on BOTH inventory RPCs. No signature change, no data
-- touched, security properties preserved (SECURITY INVOKER, locked
-- search_path = 'public', EXECUTE to `authenticated` only, `company_id` from
-- `get_my_company_id()` never the client, `products ... ORDER BY id FOR UPDATE`
-- WAC-race lock).
--
-- Three changes:
--
-- 1. ROUND AFTER SUM (the authoritative valuation contract, item 1).
--    Before: each inventory line contributed its per-movement value
--    `round(|qty| * unit_cost, 2)` to `v_je_lines`, and the per-account
--    aggregation then summed those already-rounded numbers — i.e.
--    `Σ round(line, 2)` per account. For a document with two lines of the same
--    product hitting the same account this can differ from the contract by a
--    cent per line:
--        raw 10.004 + 10.004
--        contract : round(20.008, 2)          = 20.01   ✓
--        old RPC  : round(10.004,2)+round(10.004,2) = 20.00   ✗
--    After: the RAW `|qty| * unit_cost` (full NUMERIC precision, no rounding)
--    flows into `v_je_lines`; the per-account CTE does the SINGLE `round(sum(),
--    2)`. `stock_movements.total_cost` keeps its own per-movement 2dp value —
--    that column is genuinely a per-movement figure and 2dp is correct there.
--    All arithmetic is NUMERIC; no float is introduced.
--
-- 2. `allocate_journal_number(v_company)` (migration 0033) replaces the inline
--    `select 'JE-' || lpad((count(*) + 1)::text, 4, '0') from journal_entries`
--    in both RPCs — the last two `count(*) + 1` generators are now gone; there
--    is one company-scoped, concurrency-safe, gap-safe allocator.
--
-- 3. The idempotent-reversal branch of `reverse_inventory_transaction` now
--    returns `movement_ids` (the stored correction-movement ids) and an empty
--    `warnings` array — shape-identical to the non-idempotent reversal result
--    and to both branches of `post_inventory_transaction` (item 4:
--    "idempotent reversal results contract-compatible with the normal
--    successful result ... including movement IDs").
...
declare
  ...
  v_movement_cost numeric; v_movement_value_raw numeric; v_movement_value numeric; v_new_wac numeric;
...
begin
  ...
  loop
    ...
    -- RAW (unrounded) movement value → journal aggregation (round-after-sum).
    -- ROUNDED per-movement value → the stock_movements.total_cost column only.
    v_movement_value_raw := abs(v_qty) * v_movement_cost;
    v_movement_value := round(v_movement_value_raw, 2);
    ...
    if v_inv_acct is not null and v_contra_acct is not null and v_movement_value_raw <> 0 then
      if v_qty > 0 then
        v_je_lines := v_je_lines
          || jsonb_build_object('account_id', v_inv_acct,    'debit', v_movement_value_raw, 'credit', 0)
          || jsonb_build_object('account_id', v_contra_acct, 'debit', 0, 'credit', v_movement_value_raw);
      else
        v_je_lines := v_je_lines
          || jsonb_build_object('account_id', v_contra_acct, 'debit', v_movement_value_raw, 'credit', 0)
          || jsonb_build_object('account_id', v_inv_acct,    'debit', 0, 'credit', v_movement_value_raw);
      end if;
    end if;
  end loop;

  -- journal: merge caller lines (already 2dp) + raw inventory contributions,
  -- then ONE round(sum(), 2) per account = round-after-sum.
  ...
  if v_agg is not null then
    ...
    v_je_number := public.allocate_journal_number(v_company);
