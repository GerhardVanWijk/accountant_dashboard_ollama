...
## TAX & COMPLIANCE INTEGRITY HARDENING (branch `tax-compliance-audit-2026-09-11`) — 2026-09-12

**SHIPPED 2026-09-12** — on explicit user instruction ahead of human browser QA, `tax-compliance-audit-2026-09-11` (commit `dcd9d6c`) was fast-forward-merged → `main` (`2376725..dcd9d6c`) + pushed; Cloudflare Pages auto-deploys `main` to production (`vertex-accounting.pages.dev`). **Migrations 0099–0109 applied live to the Vertex Supabase project (`bcaffvpibpitpuqglszn`), in order, with zero failures**, verified against live schema/data before and after application.

Atomic posting + idempotency logs for Income Tax, Provisional Tax, Dividends Tax, Deferred Tax, and ECL (each previously made two separate, independently-committing writes that could double-post a journal on a retry after a failure between them); statutory tax-config immutability triggers (`tax_rates`, `income_tax_year_configs`, `dividends_withholding_tax_configs`, `cgt_inclusion_rate_configs`, `cgt_annual_exclusion_configs`); related-party effective-dating + validated source-document linkage.

Migration-review hardening shipped in the same release: (1) **RPC authorization** — new `public.has_permission(feature, action)` enforces the existing Phase T permission catalog server-side for the first time, gated onto all 7 tax posting RPCs at `tax:post` (previously any authenticated company member could call them directly, regardless of role — verified live, both fixed and confirmed via rollback-wrapped tests: viewer/bookkeeper blocked, accountant/admin permitted); (2) **Tax ↔ Banking duplicate-post fix** — Provisional Tax and Dividends Tax no longer credit Cash and Bank directly; new clearing accounts `2270 Provisional Tax Payment Clearing` / `2520 Dividends Payment Clearing` (migrations 0107/0108, backfilled to existing companies + wired into new-company bootstrap) route the obligation through Banking exactly once, mirroring the existing Payroll/Leases clearing-account pattern; both RPCs reject a line that posts directly to a bank account's GL account; (3) **0102 defect fixes**, one found in review (a `_post_dividend_transition` grant/`SECURITY INVOKER` mismatch that would have broken the whole wrapper chain — fixed with an explicit lifecycle-transition allow-list instead) and one found during release smoke-testing (migration 0109 — an enum-vs-text status comparison with no cast, which would have failed on every real `declare`/`pay`/`remit` call).

Verified live: RBAC proven via rollback-wrapped synthetic-user tests (viewer/bookkeeper correctly rejected with `tax:post` missing; accountant/admin permitted); full Income Tax / Provisional Tax / Dividends Tax / Deferred Tax / ECL / Related Party lifecycles smoke-tested rollback-wrapped, zero residual data confirmed after each; Trial Balance verified balanced throughout (debits = credits = R7,634,567.50, variance R0.00); accounts 2270/2520 confirmed correctly seeded for both existing companies. 3,494/3,494 tests, type-check + lint (`--max-warnings 0`) + build all clean.

**Known limitation, deliberately out of scope for this release**: dedicated per-obligation over-settlement tracking (`subledger_settlements` + a `settle_*` RPC) for Provisional Tax / Dividends Tax, mirroring the later, separate hardening pass Payroll/Leases received (migrations 0095–0097). The duplicate-posting defect is fully closed by the clearing-account routing above; the settlement-precision layer is a recommended follow-up, not built here. **Post-deploy browser QA is now owed** (Income Tax / Provisional Tax / Dividends Tax / Deferred Tax / ECL / Compliance Dashboard / Related Party Register / Related Party Transactions — no browser automation available in-session).
